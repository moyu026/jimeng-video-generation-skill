...
```bash
python E:\pythonwork\0.study\jimeng-video-generation-skill\scripts\record_html_with_ffmpeg.py assets/html/<diagram-id>.html
```

临时覆盖动画时长：

```bash
python E:\pythonwork\0.study\jimeng-video-generation-skill\scripts\record_html_with_ffmpeg.py assets/html/<diagram-id>.html --duration 8
```
...
1. 用 Chrome `--kiosk` 全屏打开 HTML 的 `?record=1` 模式。
2. 等待页面完成首次渲染。
3. 用 ffmpeg `gdigrab` 录制 1920×1080 全屏区域。
...
- 1920×1080 页面底部至少预留 18% 高度，约 194px。
...
- 无法全屏：检查 Chrome 路径、kiosk 参数和 Windows 缩放比例；默认要求 1920×1080、100% DPI。
