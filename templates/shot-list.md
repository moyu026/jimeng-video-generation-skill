...
> 为每个 `jimeng-video` 镜头建立同名小节，并在 Prompt 中写明已确认的 `16:9` 或 `6:7`。每个即梦原始视频生成 6–8 秒，并包含逐 1–2 秒变化、镜头运动、结尾状态、风格和禁止内容；最终片段时长以对应 audioN 为准，可在后续变速。不要设计后期图文包装。
...
- [ ] 封面 Prompt 完整可执行，并指定使用 jimeng CLI 图片生成。
